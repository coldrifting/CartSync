...
public class AisleControllerTests(DatabaseSetup fixture) : DatabaseFixture(fixture)
{
    ...
    {
        ReadOnlyList<AisleResponse> aisles = await AisleController.All(SeedData.Stores[0].StoreId).ValueAsync();
        ...
        ReadOnlyList<AisleResponse> aisles2 = await AisleController.All(SeedData.Stores[1].StoreId).ValueAsync();
    ...
    public async Task TestAisleDelete()
    {
        ...
        ReadOnlyList<AisleResponse> aisles3 = await AisleController.All(SeedData.Stores[0].StoreId).ValueAsync();
        Assert.Equal(22, aisles3.Count);
        Assert.DoesNotContain(SeedData.Aisles[23].AisleId, aisles3.Select(aisle => aisle.Id));
        
        ReadOnlyList<AisleResponse> aisles4 = await AisleController.All(SeedData.Stores[1].StoreId).ValueAsync();
