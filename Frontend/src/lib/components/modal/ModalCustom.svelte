    ...
    <form onsubmit={onsubmit}>
        <div class="body">
            <div class="d-flex flex-row align-items-center justify-content-between header">
                ...
                <button type="button" onclick={hide} disabled={isLoading} class="btn-close p-2" aria-label='close'>
                </button>
            </div>
            <div class="p-5" class:d-none={!isLoading}>
                <Loading bind:isLoading />
            </div>
            <div class="content" class:d-none={isLoading}>
                {@render children()}
            </div>
        </div>
        <div class="footer w-full d-flex flex-row align-items-center justify-content-between gap-2">
            {#if actionDelete !== undefined}
                <button class="btn btn-danger me-auto" type="button" disabled={isLoading} onclick={actionDelete.action}>
                    {actionDelete.label}
                </button>
            {/if}

            <button class="btn btn-secondary ms-auto" type="button" disabled={isLoading} onclick={hide}
                    bind:this={cancelButton}>
                Cancel
            </button>
            {#if action}
                <button class="btn" class:btn-success={actionIsUpdate} class:btn-primary={!actionIsUpdate} type="submit"
                        disabled={isLoading || actionIsDisabled}>
                    {action.label}
                </button>
            {/if}
        </div>
